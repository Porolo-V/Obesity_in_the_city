...
plot_bmi_vs_composition <- function(data, sex_filter = NULL) {
  ...
  bmi_counts <- data %>%
    mutate(category = cut(BMI, breaks = c(-Inf, 18.5, 25, 30, 40, Inf), 
                          labels = c("Underweight", "Normal", "Overweight", "Obesity", "Morbid obesity"),
                          include.lowest = TRUE)) %>%
    count(category) %>%
    mutate(percent = n / sum(n) * 100)
  ...
  plot <- ggplot(data_long, aes(x = BMI, y = Value, color = Legend, shape = Legend)) +
    ...
    geom_text(data = bmi_categories, 
              aes(x = (xmin + xmax) / 2, y = 95, label = percent_label), 
...
### Function for recalculating values and plotting

```{r plot_regression_recalculation_func}
plot_regression_recalculation <- function(data, x_var, y_var, title = NA, xlab = x_var, ylab = y_var) {
  
  ols_result <- mcreg(x = data[[x_var]], y = data[[y_var]], method.reg = "LinReg")
  
  ols_slope <- round(ols_result@para[2], 2)
  ols_intercept <- round(ols_result@para[1], 2)
  
  adjusted_values_ols <- ols_slope * data[[x_var]] + ols_intercept
  
  new_col_name <- paste0(y_var, "c")
  data[[new_col_name]] <- adjusted_values_ols
  
  ols_equation <- ifelse(
    ols_intercept < 0,
    paste0(new_col_name, " = ", ols_slope, " * ", x_var, " - ", abs(ols_intercept)),
    paste0(new_col_name, " = ", ols_slope, " * ", x_var, " + ", ols_intercept))
  
  
  # CCC with 95 CI
  ccc_result <- epi.ccc(data[[x_var]], data[[new_col_name]])$rho.c
  ccc_text <- paste("CCC with 95 CI = ", round(ccc_result[1], 2), 
                    " (", round(ccc_result[2], 2), " - ", 
                    round(ccc_result[3], 2), ")", sep = "")
  
  # Pearson's coefficient and R2
  pearson_r <- cor(data[[x_var]], data[[new_col_name]], method = "pearson")
  r_squared <- round(pearson_r^2, 2)
  pearson_text <- paste0("Pearson R = ", round(pearson_r, 2), ", R² = ", r_squared)
  
  # Create plot
  plot(x = data[[x_var]], y = data[[new_col_name]], 
       main = title, 
       xlab = xlab, 
       ylab = new_col_name, 
       pch = 16,
...
```{r plot_regression_recalculation_full, fig.width=14, fig.height=5}
par(mfrow = c(1, 3))

plot_regression_recalculation(data, x_var = "FM_BIA", y_var = "FM_US")
plot_regression_recalculation(data, x_var = "BF_BIA", y_var = "BF_US")
plot_regression_recalculation(data, x_var = "FFM_BIA", y_var = "FFM_US")

mtext("Adjusted Data Plots: OLS Regression", side = 3, line = -2, outer = TRUE, cex = 1.5)
```
